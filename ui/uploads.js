 ...
 * mycomponent.getHandler().listen(this.getElement(), goog.events.EventType.CLICK, function(e) {
 *   e.stopPropagation();
 *   e.preventDefault();
 *   this.form_.trigger(); // Will open the file selection native widget supported by the browser.
 * });
 * mycomponent.getHandler().listen(this.form_, [pstj.ui.Upload.EventType.SUCCESS,
...
pstj.ui.Upload = function(url, name) {
  ...
  this.getHandler().listen(this.io_, goog.net.EventType.COMPLETE, this.handleFormComplete);
  if (goog.isString(name)) this.name_ = name;
};
goog.inherits(pstj.ui.Upload, pstj.ui.Templated);

goog.scope(function() {
  var p = pstj.ui.Upload.prototype;
  var dom = goog.dom;
  ...
  p.getTemplate = function() {
    ...
    return '<div class="' + goog.getCssName('hidden-form') +
      '"><form method="post" enctype="multipart/form-data" action="' + this.url_ +
   ...
   * @protected
   */
  p.handleFormComplete = function(e) {
  ...
  /**
   * Handles the file change event, happens when the user has selected a file
   * in the native file selector. This default implementation directly triggers
   ...
   */
  p.trigger = function() {
    dom.getElement('attachment').click();
  };
 ...
 * event.
 * @param {?Object|string} response The response from the server. Could be any, but
 * in default implementation it is expected to be a JSON object.
 */
...
pstj.ui.Upload.EventType = {
  SUCCESS: goog.events.getUniqueId(goog.DEBUG ? 'form-upload-success' : 'a'),
  FAIL: goog.events.getUniqueId(goog.DEBUG ? 'form-upload-fail': 'b')
};
